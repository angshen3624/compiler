...
(define (compile-function function)
  (let* ([instructions (cdddr function)]
    ...
    ; spill before color
    (if (list? new-function)
        (map (lambda (instruction)
               (update-stack-arg instruction (third new-function)))
             new-function)
        #f)))
...
(define (compile-L2 prog)
  (let ([final-result (compile-L2-helper prog)])
    (if (member #f final-result)
        (format "could not register allocate ~a" 
                (string->symbol
                 (substring
                  (symbol->string (first prog)) 1)))   
        (append (list (first prog))
                         final-result))))
                   
(define (compile-L2-helper prog)
