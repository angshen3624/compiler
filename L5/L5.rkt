           ...
           (if (and (null? e1-free-vars)
               ...
               (let ([new_label (fresh-label)])
                 (set!
                  lambda-function-lst
                  (cons
                   `(,new_label (,@(second e1)) ,(third e1))
                   lambda-function-lst))
                 `(,new_label ,(second e2)))
               `(let ([,x (new-tuple 0)])
                  (begin 
                    (aset ,x 0 ,(compile-e (replace-var e1 x `(aref ,x 0))))
                    ,(compile-e (replace-var e2 x `(aref ,x 0)))))))
         `(let ([,x (new-tuple 0)])
            (begin 
              (aset ,x 0 ,(compile-e (replace-var e1 x `(aref ,x 0))))
              ,(compile-e (replace-var e2 x `(aref ,x 0))))))]
...
(define (compile-L5 expr)
  (cons (compile-e expr) lambda-function-lst))
...
(pretty-display (compile-L5 (read-from-file in)))
